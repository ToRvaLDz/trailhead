...
## C. AskUserQuestion -> plain-text numbered list (the guaranteed path on Codex)
Codex's \`request_user_input\` tool only works in **Plan Mode** (Codex's collaboration mode with a plan-review step). trailhead skills run in Codex's **Default mode**: the installer never enables \`collaboration_modes\` / Plan Mode, so \`request_user_input\` is unavailable here. This is a **permanent, known constraint** of running trailhead on Codex, not a conditional "if unavailable" case.

Where the engine uses \`AskUserQuestion\`, pose the choice as a **plain-text numbered list**: on Codex this is the **guaranteed default path**, not a rare exception. Render the \`header\` and \`question\` as plain text, then one numbered line per option carrying its \`label\` (and its \`description\`): that is the \`{label, description}\` shape the engine's \`AskUserQuestion\` call-sites use, so nothing is lost in the projection. Codex has no \`multiSelect\`: for a multi-select question ask for comma-separated numbers instead of sequential single-selects. Follow \`_shared/choices.md\`'s own rules for what belongs in the list: when the call-site offers the delegate ("let the agent pick") and/or defer affordances, add them as their own numbered options under the same 4-option cap (fill real options plus applicable affordances up to 4, then push whatever does not fit into one prose line right after the list), so the numbering never diverges from what the engine intends. (You may still attempt \`request_user_input\` once first, since the call is cheap and would work if Plan Mode is ever active; treat any error or non-response from it as expected and fall through to the numbered list without waiting on it.)

After presenting the numbered list, STOP and wait: the user's **next message** is the answer to this pending question, not a fresh unrelated turn. Resolve it by matching either the **number typed** (e.g. "2") or the option's **label text** back onto that option, then continue the engine exactly as it would from a native \`AskUserQuestion\` response (e.g. write the chosen config value, or proceed down the chosen branch). If the reply matches no option, ask again rather than guessing. Never silently pick a default and proceed.
...
Codex has a hook bus, so trailhead's guardrails run as **real Codex hooks**, not degraded prose. The installer registers them in \`~/.codex/hooks.json\`: commit-guard, secret-guard, install-guard and body-guard as \`PreToolUse\` (matcher \`Bash\`) hooks that can veto the command before it runs, secret-read-guard as a \`PreToolUse\` (matcher \`Read|Bash\`) hook that denies reads of \`.env\`/\`.env.*\`/\`.secrets\`, the injection-scanner as a \`PostToolUse\` (matcher \`Bash\`) advisory, and check-update as a \`SessionStart\` hook. On Codex, file reads happen through shell commands rather than a native Read-tool event, so the secret-read guard's enforcement here runs via its **Bash** arm (protection is not lost); the \`Read\` half of its matcher is a Claude-Code-native path, registered on Codex too only so it fires without changes if a future Codex version adds an equivalent event. search-guard is Claude-Code-specific (it only prevents arming Claude's \`Read()\` deny rule under bypass permissions, a mechanism Codex lacks), so it is not projected to Codex at all. They use the same wire format they use on Claude Code (JSON on stdin, a \`decision\`/\`permissionDecision\` verdict on stdout), which Codex accepts. The host-independent git \`commit-msg\` hook is still installed at repo first-use as well (defence in depth), exactly as on Claude. Codex gates hooks behind \`features.hooks = true\` and reviews them for trust on first start: the installer enables the flag, and Codex will ask you to trust trailhead's hooks the next time it starts.
...
// --- MODEL_KEYS --------------------------------------------------------------
// The 7 pinnable technique keys, mirroring the models.codex.* namespace. The
// other committed agent (fix) has no pin key and is always
// projected pin-less (session model).
const MODEL_KEYS = new Set(['plan', 'execute', 'research', 'review', 'debug', 'verify', 'codebase-map']);
...
// review/debug/verify/codebase-map) carries its models.codex.<key> pin when set, else is
// pin-less; the 1 keyless agent (fix) is always pin-less.
...
// --- codexHookEntries -----------------------------------------------------
// The six guardrail hooks trailhead registers on Codex, as {event, matcher,
...
// secret, install, body, secret-read) can veto; the injection-scanner
// is a PostToolUse advisory; check-update runs at SessionStart. Reused by the
// installer, which merges them into ~/.codex/hooks.json (same shape as
// Claude's settings.json hooks block).
// search-guard is deliberately NOT projected here: it is Claude-Code-specific
// (its only job is to stop a `cd <dir> && grep <relative>` shell shape from
// arming Claude's `Read()` deny rule under bypass permissions), a permission
// mechanism Codex does not have.
function codexHookEntries(hooksScriptsDir) {
  ...
  return [
    ...
    { event: 'PreToolUse', matcher: 'Bash', command: cmd('trailhead-body-guard.js') },
    { event: 'PreToolUse', matcher: 'Read|Bash', command: cmd('trailhead-secret-read-guard.js') },
