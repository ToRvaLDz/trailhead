...
- **Native sub-issue**: every ticket is a **sub-issue of its map** (`gh api --method POST repos/{owner}/{repo}/issues/<map>/sub_issues -F sub_issue_id=<ticket .id>`). This is the structure GitHub renders (the map shows its children + a progress bar); it carries the `Parent:` line's meaning as a real edge. Native sub-issues are capped at 100 per map by GitHub; `/trailhead:prune` reclaims slots by dropping closed/orphan edges while keeping every reference, and because the frontier keys off the `trailhead:map-<n>` label this never affects engine behaviour.
