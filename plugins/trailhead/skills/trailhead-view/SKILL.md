...
**Exhausted-map advisory on the self-heal entry points.** Because bare `/trailhead`, the `map` render, and the `whiteboard` render already touch the dashboard to self-heal the pin, each also runs the cheap per-map `0 open scoped issues?` count and, if it finds an **exhausted-but-open** map, surfaces a one-line `exhausted · closeable` advisory with the offer to close it (never auto-close, and never a body rewrite here). This keeps the offer recurring across every read-only entry point, not just `/trailhead:dashboard`, so a missed exhaustion moment always gets a second chance. (The `map` render's own **Exhaustion check** already handles the map it is rendering; this advisory covers any other exhausted-but-open map.)
