...
The full config **key reference** is currently duplicated between this cluster's `references/configuration.md` (keys + guided setup) and the eager core's `../_shared/configuration-reference.md` (the two are byte-identical today); they are deduped to a single source at the downstream `_shared/` cutover ([map #53](https://github.com/ToRvaLDz/trailhead/issues/53)).
