...
3. **Fix** — implement the correction with **atomic commits** (per the repo's `trailhead:conventions` `git:` — `main` by default, else a branch + PR), via an **executor subagent on `config.models.execute`** (inline when unset/same as session). Where sensible, a test that fails before and passes after (**TDD**).
