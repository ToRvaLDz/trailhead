...
- **A verb** to hand off to the skill that owns it, passing the whole argument string. **During the skill-split migration ([map #53](https://github.com/ToRvaLDz/trailhead/issues/53)) the cluster skills (chart / work / view / capture / manage) are not built yet, so every verb routes to the `trailhead-monolith` skill**: call the Skill tool with skill name `trailhead-monolith` and arguments `<verb> <rest>`, then carry out its instructions. As each cluster skill lands (#59 to #63), its verbs repoint from `trailhead-monolith` to that cluster.
