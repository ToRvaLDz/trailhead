...
- **Out of scope**: what's been ruled out; **flag as an advisory** any line that's actually **deferred** rather than truly beyond the destination, i.e. one that **names a gate/trigger** (*gated by X*, *quando X*, *dipende da X*) or is a **feature wanted later** (another milestone/map). Say so and point at the re-route (seed/idea/todo via `inbox` or an explicit go-ahead); don't stay silent when such a line is present. See [Out of scope](#out-of-scope).
...
2. **Map the frontier.** Grill again, **breadth-first**: fan out across the whole space rather than deep on one thread, surfacing the open decisions and the first steps takeable now. **If no fog surfaces** (the way is already clear, the whole thing fits one session) you don't need a map: stop and ask how to proceed. **Harvest parked todos:** if **earlier maps on this repo** left *Out of scope* lines annotated `→ future map` (deferred todos, see [Out of scope](#out-of-scope)), read them and **ask the user which belong in *this* map**; pull the chosen ones in as tickets, and leave (or update the destination on) the rest.
...
4. **Map the frontier of the remainder** → tickets specifiable now + fog in *Not yet specified*, then wire the blocking and fire the research. **Harvest parked todos** from earlier maps on this repo (their `→ future map` *Out of scope* lines), asking the user which belong here, as in Mode 1 step 2. As Mode 1, steps 4–6.
