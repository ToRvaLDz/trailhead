...
Load the effective config at the start of a work session. **Then check the session model against `config.models.execute`:** the Execute step runs inline in this interactive session (it's HITL — it can't be a subagent without losing "stop and ask"), so a session can't apply `models.execute` from inside — it's the model the session must be *launched* on. If your own model differs from `config.models.execute` before working a `build`/`bug` ticket, **surface the mismatch** (the user relaunches or `/model`s onto it, or you proceed noting the deviation) — never silently execute on the wrong model. The other model keys (`plan`/`research`/`review`/`debug`) run as subagents and apply automatically. The **keys, their values, the model/tdd/acceptance semantics, and the guided menu setup live in `references/configuration.md`** — read it when running `/trailhead:config` or when you need a key's exact meaning. `/trailhead:config` with no args runs the guided setup; `config get` prints the merged config; `config set <key> <value>` writes one key.
