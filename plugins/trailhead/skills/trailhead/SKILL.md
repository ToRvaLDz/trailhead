...
- **Per-map label `trailhead:map-<n>`** (n = the map issue's own number): on the map issue **and every one of its tickets**. This is the **queryable key** that scopes the frontier to one query, because native sub-issues aren't cheaply filterable in a single search (exactly as with `blocked_by`), so the label carries the query.
