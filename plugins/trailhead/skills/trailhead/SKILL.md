...
| Conventions | a **single per-repo** issue with label `trailhead:conventions`: the project's **way of working**, readable by everyone. Repo-scoped and linked from every map's Notes, like Codebase. It opens with a small machine-read header the engine obeys, then human prose. Header keys (defaults **bold**): `git:` **`main`** \| `pr` (commit straight to `main`, or feature branch + PR) · `release:` **`command`** \| `auto` (never release without an explicit command, or release automatically per the project's flow) · `isolation:` **`none`** \| `worktree` (work in the current checkout, or give each executing ticket its own `git worktree` + `trailhead/t<n>` branch so concurrent sessions on one clone never share a working tree; see [Working as a team](#working-as-a-team)). **`worktree` implies a branch per ticket even under `git: main`** (git won't check the trunk out twice); the branch integrates to the trunk per `git:` at Resolve. Below the header: the standing "how we work here" notes. **Filled at chart/adopt by a brief conventions brainstorming** (see the Invocation modes), not a `trailhead:ticket`, off the frontier. |
...
2. Choose the ticket. If the user names one, use it. Otherwise the first frontier ticket in order. **Claim** it: assign it to yourself before any work, then re-read the assignee before starting; on a collision, stop and ask the user (see [Working as a team](#working-as-a-team)). If the ticket proves too big once you're in it, **split** it rather than grind (same section).
   - **Then, under `isolation: worktree`, set up the worktree NOW, before any file edit or diagnostic probe** (not later at Execute/Fix): create/enter this ticket's `git worktree` on a `trailhead/t<n>` branch for the repo the ticket's `Scope:` points at (**the submodule itself for submodule-scoped work**, e.g. `git -C app worktree add ../<repo>-t<n> -b trailhead/t<n>`), and do every subsequent step there. This is the whole point of isolation: if you diagnose and edit in the shared checkout "just to start", a concurrent session is already colliding with you. Do it silently (don't narrate it); it's undone at Resolve when the branch integrates and the worktree is removed. See [Working as a team](#working-as-a-team).
