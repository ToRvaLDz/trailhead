...
| Element | How |
|----------|------|
| Map | an issue with label `trailhead:map`. **Pin it**, along with the codebase and conventions issues — GitHub's **3 pinned-issue slots** are exactly map + codebase + conventions, so the repo's trailhead anchors stay one click away (`gh api graphql` `pinIssue` mutation on each issue's node id). |
