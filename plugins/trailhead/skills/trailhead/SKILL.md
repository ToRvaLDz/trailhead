...
- **No map narrative book-keeping.** A whiteboard ticket has no map, so there is no `Decisions so far` update, no fog to graduate, no frontier re-scan. Everything else holds: atomic commits with `Refs: #<n>`, TDD / Code review / Acceptance testing per the cycle, the resolution comment + `gh issue close`, and the [Session handoff](#session-handoff). **Exception, `quick <n>` on a *map* ticket: dependent-unblocking still runs** (remove `trailhead:blocked` from any dependent whose last blocker just closed, [Mode 2](#mode-2-work-the-map-trailheadwork) step 4); skipping it would strand those dependents off the frontier for good. Only the *narrative* book-keeping above and the split are skipped, so prefer `work <n>` for a map ticket that has dependents or belongs in the map record.
- **Same isolation setup as `work`.** Under `isolation: worktree`/`clone`, `quick` sets up the ticket's `trailhead/t<n>` workspace before any edit, re-checks the claim, and writes the `.trailhead/session-ticket` marker, exactly as [Mode 2](#mode-2-work-the-map-trailheadwork) step 2 (no active-map marker: a whiteboard ticket has no map). Under `isolation: none`, the scope-free check applies too.
