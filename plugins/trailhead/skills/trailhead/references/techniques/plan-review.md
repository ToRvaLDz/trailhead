...
2. **Send the same brief to each reviewer.** Give every CLI the ticket's `## Question`, the drafted PLAN, and the map's relevant Notes/decisions in a **self-contained prompt**, so the reviewer needn't explore the repo. Ask for a short list of concerns (*risks, missing steps, wrong assumptions, simpler alternatives*) each tagged **blocking** or **non-blocking**.
   ...
   - **Fast posture.** Make the call quick and non-interactive so it can't stall on an approval prompt or a heavy reasoning pass. For **codex**: `codex exec -s read-only -c model_reasoning_effort=low "<prompt>"` (read-only = nothing to approve; low effort = quick), or a preconfigured profile with `-p <profile>`. For **gemini**: `gemini -p "<prompt>"`; **qwen**: `qwen -p "<prompt>"`. Verify each tool's exact headless flag and config key for your version.
   - **Generous timeout + one retry.** Allow **a few minutes** per call (not 60s). On the **first** timeout, retry once with more margin; only on a second timeout or a hard error do you **drop** that reviewer and continue. A slow reviewer must never wedge or fail the build.

   (This mirrors how GSD avoids the timeout: it pins each reviewer CLI to a fast "passive posture" once, in the CLI's own config, e.g. codex's `~/.codex/config.toml` model + reasoning effort, so every review call is quick by default. Setting such a profile once is the durable fix; the inline flags above are the per-call equivalent.)
