...
1. **Detect reviewers.** `command -v` for external AI CLIs on the PATH — `gemini`, `codex`, `qwen`, `opencode`, `cursor-agent`, `coderabbit` (and any OpenAI-compatible local server you've configured). If `config.plan_review` names specific ones, use only those. **If none are available, note it in the PLAN comment and skip — never fail the build over a missing reviewer.**
