...
Se l'MCP Claude Design completo non è connesso, trailhead **offre di installarlo una volta** (`claude mcp add ... claude-design`, poi `/design-login`); se rifiuti, `canvas` ripiega sul percorso design-system o sul disco locale. Su Codex (nessun MCP Claude Design) è sempre disco locale, senza offerta. `design.approval` decide se il build attende il tuo esplicito via libera (`explicit`), o procede subito dopo aver mostrato il mockup (`auto`); non tocca l'offerta di installazione una tantum, che compare in entrambi i casi.
