...
`/trailhead:config` runs a guided, menu-driven setup: pick the scope, then walk each setting (way of working (git, release, isolation), ticket language, models, design + approval, TDD, effort, acceptance testing, plan review, statusline) as an icon-labelled menu; no hand-editing JSON. Every step is asked (none skipped), and plan and execute models are always two separate, version-pinned choices. `config get` prints the effective merged config; `config set <key> <value>` writes one key.
...
| `effort` | **`standard`** \| `lean` | how hard the `build`/`bug` engine works a change: `standard` runs every step, `lean` size-triages small changes and offers to skip heavy ones (plan pass, TDD, cross-AI review, full code review, goal-backward verify). Governs `build`/`bug` only: `quick` always size-triages regardless of this key |

`git`, `release`, and `isolation` aren't config.json keys: they're conventions-header keys stored in the repo's `trailhead:conventions` issue, reachable from the guided setup's way-of-working step (see [Core concepts](/docs/concepts) for the full header).
