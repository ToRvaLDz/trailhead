...
| `effort` | **`standard`** \| `lean` | how hard the `build`/`bug`/`quick` engine works a change: `standard` runs every step, `lean` size-triages small changes and offers to skip heavy ones (plan pass, TDD, cross-AI review, full code review) |
