...
Beyond the skill's instructions, trailhead ships four of its own hooks (in `hooks/`, self-contained via `${CLAUDE_PLUGIN_ROOT}`) that *enforce* the parts of the discipline the model shouldn't be trusted to remember:
...
All four are crash-safe (any error → allow) and active whenever the plugin is installed. The commit, secret, and install guards therefore apply in **every** repo, not only trailhead projects: intentional, since conventional commits, no `Co-Authored-By`, no leaked secrets, and no unvetted installs are trailhead's standing rules. To opt out, disable the plugin's hooks in your Claude Code settings.
